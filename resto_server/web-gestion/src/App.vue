...
const AUTH_STORAGE_KEY = 'restobar_web_internal_token';
const gatewayDisplayUrl = window.location.origin;
...
const mesaForm = reactive({ numero: '' });
...
const mesaActionLabel = computed(() => {
  const mesaNumero = String(mesaForm.numero ?? '').trim();
...
async function parseResponsePayload(response) {
  ...
  try {
    ...
    throw new Error('El gateway devolvio una respuesta invalida');
  }
}
...
function scheduleInternalRealtimeReconnect() {
  if (internalSocketReconnectId || internalSocketClosedManually || !token()) {
    return;
  }
...
function openInternalRealtimeChannel() {
  ...
  const socket = new WebSocket(
    buildGatewayWebSocketUrl(`/api/internal/socket?token=${encodeURIComponent(token())}`),
  );
...
async function loadRoleData() {
  ...
  try {
    if (authSession.value.role === 'mozo') {
      ...
      await Promise.all([loadDashboard(), loadCategorias(), loadProductos(), loadMenuImages()]);
    }
  } catch (error) {
    showError(error.message);
  } finally {
...
async function restoreSession() {
  ...
  try {
    const payload = await fetchJson('/api/internal/auth/session');
...
async function editProduct(product) {
  ...
  productForm.categoriaId = String(product.categoriaId);
...
async function toggleMesa(mesa) {
  loading.value = true;
  clearFeedback();
  try {
    await fetchJson(`/api/internal/mesas/${mesa.id}`, {
      method: 'PUT',
      body: JSON.stringify({
        numero: mesa.numero,
    ...
    await loadMesas();
    showInfo('La mesa fue actualizada correctamente.');
  } catch (error) {
    showError(error.message);
  } finally {
    loading.value = false;
...
async function closeMesa(mesa) {
  if (!window.confirm(`Deseas cerrar la mesa ${mesa.numero}?`)) {
    return;
  }
  loading.value = true;
  clearFeedback();
  try {
    await fetchJson(`/api/internal/mesas/${mesa.numero}/close`, {
      method: 'POST',
      body: JSON.stringify({}),
    });
  ...
  <main class="layout">
    <header class="hero">
      ...
      <div class="login-card" v-else>
        <span>Canal</span>
        <strong>{{ gatewayDisplayUrl }}</strong>
        <span class="login-card-label-secondary">Carta online</span>
        <input
          class="readonly-url-input"
          :value="tunnelInfo?.publicUrl || 'Tunel efimero iniciando...'"
              ...
              <article v-for="column in columns" :key="column.key" class="column-card">
                ...
                <div v-if="column.key === 'consultas' && detail.consulta" class="detail">
                  ...
                  <div class="chat-thread">
                    <article v-for="mensaje in detail.consulta.mensajes" :key="mensaje.id" class="chat-message" :class="{ 'chat-message-own': mensaje.autorTipo === 'mozo' }">
                      <strong>{{ mensaje.autorTipo === 'mozo' ? 'Mozo' : 'Cliente' }}</strong>
                ...
                <div v-else-if="column.key === 'pedidosCocina' && detail.pedidoCocina" class="detail">
                  ...
                  <ul class="detail-list">
                    <li v-for="item in detail.pedidoCocina.items" :key="`${item.titulo}-${item.clienteSesionId}`">{{ item.titulo }} x{{ item.cantidad }} ({{ item.clienteSesionId }})</li>
                  </ul>
            ...
            <article class="admin-card">
              ...
              <form class="simple-form" @submit.prevent="createMesa">
                <input
                  ...
                  type="number"
                  min="1"
                  placeholder="Numero exacto de mesa, por ejemplo 4"
                />
              ...
              <div class="table-list">
                <article v-for="mesa in mesas" :key="mesa.id" class="table-row">
                  <div>
                    ...
                    <p>{{ mesa.habilitada ? 'Habilitada' : 'Deshabilitada' }} - {{ mesa.sesionActiva ? 'Sesion activa' : 'Sin sesion activa' }}</p>
                  </div>
                  <div class="row-actions">
                    <button class="ghost-button" :disabled="loading" @click="toggleMesa(mesa)">{{ mesa.habilitada ? 'Deshabilitar' : 'Habilitar' }}</button>
          ...
          <section class="panel-block secondary-grid">
            ...
            <article class="admin-card">
              ...
              <form class="simple-form stacked-form" @submit.prevent="createProduct">
                <select v-model="productForm.categoriaId">
                  <option disabled value="">Categoria</option>
                  <option v-for="category in categorias" :key="category.id" :value="category.id">{{ category.titulo }}</option>
                </select>
              ...
              <div class="table-list">
                <article v-for="product in productos" :key="product.id" class="table-row">
                  <div class="product-row-copy">
                    ...
                    <p>{{ product.categoriaTitulo }} - {{ formatMoney(product.precioArsCentavos) }}</p>
