...
function getDomainUrl(pathname, searchParams = null) {
  const url = new URL(pathname, process.env.GATEWAY_DOMAIN_BASE_URL);
  if (searchParams) {
...
function getDomainEventsUrl() {
  return new URL('/internal/events/stream', process.env.GATEWAY_DOMAIN_BASE_URL);
}
...
async function publishSnapshotToSubscriber(subscriber) {
  try {
    const payload = await callDomain(
      `/internal/public/mesas/${subscriber.mesaNumero}/state`,
      {},
...
function handleDomainNotification(message) {
  ...
  try {
    ...
    app.log.warn({ message }, 'Se ignoro una notificacion de dominio invalida en el gateway');
    return;
  }
...
function scheduleDomainEventReconnect(reason) {
  ...
  domainEventReconnectTimer = setTimeout(() => {
    ...
    startDomainEventStream().catch((error) => {
      app.log.error(error);
      scheduleDomainEventReconnect('error-en-reconexion-gateway');
    });
...
async function startDomainEventStream() {
  ...
  try {
    ...
    if (!response.ok || !response.body) {
      throw new Error('El domain rechazo el stream interno de eventos del gateway');
    }
...
async function readTunnelState() {
  try {
    ...
    const fallbackUrl = process.env.CLOUDFLARE_PUBLIC_CLIENT_URL ?? null;

    return {
      service: 'tunel-service',
      ready: Boolean(fallbackUrl),
      publicUrl: buildPublicEntrypointUrl(fallbackUrl, '/carta'),
      targetUrl: null,
      startedAt: null,
      lastOutputAt: null,
      lastError: error.message,
      source: fallbackUrl ? 'fallback-env' : 'tunel-service-unavailable',
    };
  }
}
...
async function requireInternalSessionByToken(sessionToken, allowedRoles) {
  if (!sessionToken) {
    const error = new Error('Falta la sesion interna');
...
app.setErrorHandler((error, request, reply) => {
  ...
  reply.code(error.statusCode ?? 500).send({
    error: error.message ?? 'Ocurrio un error interno en el gateway',
  });
});
...
app.get('/api/public/bootstrap', async () => {
  ...
  return {
    ...
    businessTimezone: process.env.BUSINESS_TIMEZONE,
    visualUsdExchangeRate: process.env.VISUAL_USD_EXCHANGE_RATE ?? null,
  };
});
...
app.post('/api/public/mesas/:mesaNumero/session', async (request, reply) => {
  const payload = await callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/session`,
    {
      method: 'POST',
      body: JSON.stringify({
...
app.get('/api/public/mesas/:mesaNumero/menu', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/menu`,
    {},
    {
...
app.post('/api/public/mesas/:mesaNumero/disconnect', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/disconnect`,
    {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: request.body?.clientSessionId ?? null,
...
app.get('/api/public/mesas/:mesaNumero/state', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/state`,
    {},
    {
      clientSessionId: request.query?.clientSessionId ?? null,
    },
...
app.post('/api/public/mesas/:mesaNumero/cart/items', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/cart/items`,
    {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: request.body?.clientSessionId ?? null,
...
app.post('/api/public/mesas/:mesaNumero/order/confirm', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/order/confirm`,
    {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: request.body?.clientSessionId ?? null,
...
app.post('/api/public/mesas/:mesaNumero/waiter-call', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/waiter-call`,
    {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: request.body?.clientSessionId ?? null,
...
app.post('/api/public/mesas/:mesaNumero/consulta/open', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/consulta/open`,
    {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: request.body?.clientSessionId ?? null,
...
app.post('/api/public/mesas/:mesaNumero/consulta/message', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/consulta/message`,
    {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: request.body?.clientSessionId ?? null,
...
app.post('/api/public/mesas/:mesaNumero/consulta/close', async (request) =>
  callDomain(
    `/internal/public/mesas/${request.params.mesaNumero}/consulta/close`,
    {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: request.body?.clientSessionId ?? null,
...
app.get('/api/public/mesas/:mesaNumero/socket', { websocket: true }, async (socket, request) => {
  ...
  try {
    await callDomain(
      `/internal/public/mesas/${mesaNumero}/connect`,
      {
        method: 'POST',
        body: JSON.stringify({
          clientSessionId,
  ...
  const disconnectMesaClient = async () => {
    ...
    try {
      await callDomain(
        `/internal/public/mesas/${mesaNumero}/disconnect`,
        {
          method: 'POST',
          body: JSON.stringify({
            clientSessionId,
...
app.get('/api/internal/auth/session', async (request) =>
  callDomain('/internal/auth/session', {
    headers: {
      Authorization: getAuthorizationHeader(request) ?? '',
    },
  }));

app.post('/api/internal/auth/logout', async (request) =>
  callDomain('/internal/auth/logout', {
    method: 'POST',
    headers: {
      Authorization: getAuthorizationHeader(request) ?? '',
    },
...
app.get('/api/internal/socket', { websocket: true }, async (socket, request) => {
  const sessionToken = getSessionTokenValue(request.query?.token ?? null);
  let subscriber = null;

  try {
    const session = await requireInternalSessionByToken(sessionToken, ['mozo', 'encargado']);
...
app.get('/api/internal/shell', async (request) => {
  ...
  return {
    ...
    domainBaseUrl: process.env.GATEWAY_DOMAIN_BASE_URL,
  };
});
...
app.put('/api/internal/mesas/:mesaId', async (request) => {
  await requireInternalSession(request, ['mozo']);
  return callDomain(`/internal/admin/mesas/${request.params.mesaId}`, {
    method: 'PUT',
    headers: {
      Authorization: getAuthorizationHeader(request) ?? '',
    },
...
app.post('/api/internal/mesas/:mesaNumero/close', async (request) => {
  await requireInternalSession(request, ['mozo']);
  return callDomain(`/internal/admin/mesas/${request.params.mesaNumero}/close`, {
    method: 'POST',
    headers: {
      Authorization: getAuthorizationHeader(request) ?? '',
    },
...
await app.listen({
  ...
  port: Number.parseInt(process.env.GATEWAY_PORT, 10),
});

startDomainEventStream().catch((error) => {
  app.log.error(error);
  scheduleDomainEventReconnect('error-en-arranque-gateway');
});
