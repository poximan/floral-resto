...
async function refreshMenu() {
  const payload = await fetchJson(
    `/api/public/mesas/${mesaNumero.value}/menu?clientSessionId=${encodeURIComponent(clientSessionId.value)}`,
  );
...
function openMesaRealtimeChannel() {
  ...
  const socket = new WebSocket(
    buildGatewayWebSocketUrl(
      `/api/public/mesas/${mesaNumero.value}/socket?clientSessionId=${encodeURIComponent(clientSessionId.value)}`,
    ),
  );
...
async function requestMesaSession(numeroMesa) {
  ...
  return fetchJson(`/api/public/mesas/${numeroMesa}/session`, {
    method: 'POST',
    body: JSON.stringify({
...
async function submitMesaNumber() {
  const numeroMesa = Number.parseInt(mesaNumeroInput.value, 10);

  if (!Number.isInteger(numeroMesa) || numeroMesa <= 0) {
    errorMessage.value = 'Ingresa un numero de mesa valido.';
    return;
  }
...
async function updateCart(productoId, action) {
  ...
  try {
    ...
    try {
      payload = await fetchJson(`/api/public/mesas/${mesaNumero.value}/cart/items`, {
        method: 'POST',
        body: JSON.stringify({
...
async function confirmOrder() {
  ...
  try {
    const payload = await fetchJson(`/api/public/mesas/${mesaNumero.value}/order/confirm`, {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: clientSessionId.value,
      }),
...
async function submitConsulta() {
  ...
  try {
    const payload = await fetchJson(`/api/public/mesas/${mesaNumero.value}/consulta/open`, {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: clientSessionId.value,
...
async function sendConsultaMessage() {
  ...
  try {
    const payload = await fetchJson(`/api/public/mesas/${mesaNumero.value}/consulta/message`, {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: clientSessionId.value,
...
async function closeConsulta() {
  ...
  try {
    const payload = await fetchJson(`/api/public/mesas/${mesaNumero.value}/consulta/close`, {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: clientSessionId.value,
      }),
...
async function callWaiter() {
  ...
  try {
    const payload = await fetchJson(`/api/public/mesas/${mesaNumero.value}/waiter-call`, {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: clientSessionId.value,
      }),
  ...
  <main class="layout">
    <header class="hero">
      <div>
        ...
        <h1>Carta digital</h1>
        <p class="subtitle">
          {{ hasMesa ? 'Explora la carta, arma el pedido compartido de la mesa y consulta al mozo si lo necesitas.' : 'Escanea, ingresa tu numero de mesa y empieza a pedir.' }}
        </p>
      </div>
    ...
    <section v-if="!hasMesa" class="mesa-form">
      <h2>Bienvenido</h2>
      <p>Indica el numero de mesa para cargar la carta compartida.</p>
      <div class="mesa-form-row">
        <input v-model="mesaNumeroInput" type="number" min="1" placeholder="Mesa" />
      ...
      <section v-else class="state-card">
        ...
        <section v-if="activeConsulta" class="consulta-panel">
          ...
          <div class="consulta-thread">
            ...
            >
              <strong>{{ mensaje.autorTipo === 'cliente' ? 'Cliente' : 'Mozo' }}</strong>
