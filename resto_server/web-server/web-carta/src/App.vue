...
const canConfirmComanda = computed(() => mesaState.value?.canConfirmComanda === true);
const confirmedComandasCount = computed(() => mesaState.value?.totalComandasConfirmadas ?? 0);
...
const restaurantLogoUrl = '/assets/resto_icon.png';
...
function formatMoney(arsCentavos) {
  ...
  if (selectedCurrency.value === 'USD' && visualUsdExchangeRate.value > 0) {
    ...
    return formatCurrencyWithoutUnitGap(new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: 'USD',
  ...
  return formatCurrencyWithoutUnitGap(new Intl.NumberFormat('es-AR', {
    style: 'currency',
    currency: 'ARS',
    minimumFractionDigits: 2,
  }), arsValue);
...
function formatCurrencyWithoutUnitGap(formatter, value) {
  const parts = formatter.formatToParts(value);

  return parts
    .filter((part, index) => !(part.type === 'literal'
      && /^[\s\u00a0]+$/.test(part.value)
      && parts[index - 1]?.type === 'currency'
      && parts[index + 1]?.type === 'integer'))
    .map((part) => part.value)
    .join('');
}
...
function syncMenuCountersWithState() {
  const quantityByProductId = new Map(
    (mesaState.value?.comandaActiva?.items ?? []).map((item) => [item.productoId, item.cantidadTotal]),
  );
...
function toggleCategoryCollapse(categoryId) {
  if (isCategoryCollapsed(categoryId)) {
    collapsedCategoryIds.value = collapsedCategoryIds.value.filter((id) => id !== categoryId);
    return;
  }

  collapsedCategoryIds.value = [...collapsedCategoryIds.value, categoryId];
}
...
function buildRemovedProductsMessage(removedProducts) {
  ...
  if (titles.length === 0) {
    return 'Uno o mas productos fueron retirados del menu y se eliminaron de tu comanda.';
  }

  return `Se retiraron del menu y se eliminaron de tu comanda: ${titles.join(', ')}.`;
}
...
async function applyMesaState(nextState, options = {}) {
  ...
  const removedProductsMessage = buildRemovedProductsMessage(nextState?.productosRemovidosDeComanda ?? []);
...
function productCounterQuantity(producto) {
  return Number(producto.cantidadTotalMesa ?? 0);
}

async function updateComanda(producto, action) {
  const productoId = producto.id;

  if (action === 'add' && productCounterQuantity(producto) >= maxProductCounterQuantity) {
    errorMessage.value = `El contador admite hasta ${maxProductCounterQuantity} unidades por producto.`;
    return;
  }
  ...
  try {
    ...
    try {
      payload = await fetchJson(`/api/public/mesas/${mesaPathSegment()}/comandas/items`, {
        method: 'POST',
        body: JSON.stringify({
          clientSessionId: clientSessionId.value,
          productoId,
...
async function confirmComanda() {
  ...
  try {
    const payload = await fetchJson(`/api/public/mesas/${mesaPathSegment()}/comandas/confirm`, {
      method: 'POST',
      body: JSON.stringify({
        clientSessionId: clientSessionId.value,
      }),
    ...
    categorias.value = payload.menu;
    await applyMesaState(payload.state);
    infoMessage.value = 'La comanda se confirmo correctamente.';
  ...
  <main v-else class="layout">
    <header class="hero">
      <div>
        <div class="eyebrow-row">
          ...
          <section v-if="hasMesa && selectedTab === 'menu'" class="currency-toggle" aria-label="Moneda de visualizacion">
            <button
              class="tab"
        ...
        <h1>Carta digital</h1>
        <p class="subtitle">
          {{ hasMesa ? 'Explora la carta, arma la comanda compartida de la mesa y consulta al mozo si lo necesitas.' : 'Escanea, ingresa el nombre de mesa y empieza a pedir.' }}
        </p>
      </div>
    ...
    <template v-else-if="!mesaClosedMessage">
      ...
      <section class="session-banner" :class="{ 'session-banner-leader': isLeader }">
        <strong v-if="isLeader">Tu dispositivo confirma la comanda de la mesa.</strong>
        <strong v-else>Otra persona de la mesa confirma la comanda.</strong>
        <p>
          {{ isLeader ? 'Puedes seguir cargando items y confirmar la comanda pendiente cada vez que la mesa lo necesite.' : 'Puedes sumar o quitar items, pero la confirmacion final la hace quien lidera la mesa.' }}
        </p>
      </section>
      ...
      <section v-if="selectedTab === 'menu'" class="catalog">
        <article v-for="categoria in categorias" :key="categoria.id" class="category">
          <header class="category-header">
            ...
            <button class="ghost-button category-collapse-button" type="button" @click="toggleCategoryCollapse(categoria.id)">
              {{ isCategoryCollapsed(categoria.id) ? 'Mostrar' : 'Ocultar' }}
            </button>
          </header>
          <div v-if="!isCategoryCollapsed(categoria.id)" class="category-body">
            <section v-for="subcategoria in categoria.subcategorias" :key="subcategoria.id" class="subcategory">
              <h3 class="subcategory-title">{{ subcategoria.titulo }}</h3>
              <div class="subcategory-products">
                <div v-for="producto in subcategoria.productos" :key="producto.id" class="product">
                  <div class="product-main">
                    <div class="product-header-row">
                      <h3 class="product-title">{{ producto.titulo }}</h3>
                      <div class="counter product-counter">
                        <button
                          class="ghost-button counter-button"
                          :disabled="loading || productCounterQuantity(producto) <= 0"
                          @click="updateComanda(producto, 'remove')"
                        >
                          -
                        </button>
                        <span>{{ productCounterQuantity(producto) }}</span>
                        <button
                          class="primary-button counter-button"
                          :disabled="loading || productCounterQuantity(producto) >= maxProductCounterQuantity"
                          @click="updateComanda(producto, 'add')"
                        >
                          +
                        </button>
                      </div>
                      <span class="product-price">{{ formatMoney(producto.precioArsCentavos) }}</span>
                    </div>
                    <div class="product-detail-row">
                      <p class="product-description">{{ producto.descripcion }}</p>
                      <button
                        class="ghost-button product-consult-button"
      ...
      <section v-else class="state-card">
        ...
        <p><strong>Comandas enviadas:</strong> {{ confirmedComandasCount }}</p>
        <p>
        ...
        <h3>Comanda pendiente</h3>
        <div class="state-items">
          <article
            v-for="item in mesaState?.comandaActiva?.items ?? []"
            :key="item.productoId"
            class="state-item"
          >
            <div>
          ...
          <article v-if="!(mesaState?.comandaActiva?.items?.length)" class="state-item">
            <div>
              <strong>Sin items pendientes</strong>
              <p>Cuando agreguen productos a la comanda compartida apareceran aqui hasta que el lider confirme el envio.</p>
            </div>
          </article>
        </div>

        <p class="state-total">
          Total pendiente: {{ formatMoney(mesaState?.comandaActiva?.totalArsCentavos ?? 0) }}
        </p>

        <section v-if="mesaState?.comandasMesa?.length" class="confirmed-orders">
          <h3>Comandas de la mesa</h3>
          <article
            v-for="comanda in mesaState?.comandasMesa ?? []"
            :key="comanda.id"
            class="confirmed-order-card"
          >
            <header class="confirmed-order-header">
              <strong>Comanda {{ comanda.numeroOrden }}</strong>
              <span>{{ new Date(comanda.confirmadaEn).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit', hour12: false }) }}</span>
            </header>
            <div class="state-items">
              <article
            ...
            <p class="state-total">
              Total mesa: {{ formatMoney(comanda.totalArsCentavos) }}
            </p>
          </article>
        </section>
        ...
        <section class="confirmed-orders">
          <h3>Mis consumos</h3>
          <article
            v-if="!(mesaState?.comandaActiva?.misItems?.length) && !(mesaState?.misComandas?.length)"
            class="state-item"
          >
            <div>
              <strong>Sin consumos propios</strong>
              <p>Los items que pidas desde esta carta apareceran separados en esta seccion.</p>
            </div>
          </article>
          <article v-if="mesaState?.comandaActiva?.misItems?.length" class="confirmed-order-card">
            <header class="confirmed-order-header">
              <strong>Comanda pendiente</strong>
              <span>{{ formatMoney(mesaState?.comandaActiva?.miTotalArsCentavos ?? 0) }}</span>
            </header>
            <div class="state-items">
              <article
                v-for="item in mesaState?.comandaActiva?.misItems ?? []"
                :key="`activa-${item.productoId}`"
                class="state-item"
              >
                <div>
                  <strong>{{ item.titulo }}</strong>
          ...
          <article
            v-for="comanda in mesaState?.misComandas ?? []"
            :key="`mia-${comanda.id}`"
            class="confirmed-order-card"
          >
            <header class="confirmed-order-header">
            ...
            <div class="state-items">
              <article
                v-for="item in comanda.items"
                :key="`mia-${comanda.id}-${item.productoId}`"
                class="state-item"
              >
                <div>
                  <strong>{{ item.titulo }}</strong>
            ...
            <p class="state-total">
              Total propio: {{ formatMoney(comanda.totalArsCentavos) }}
            </p>
          </article>
        </section>
      </section>
      ...
      <footer class="sticky-footer">
        ...
        <button
          class="ghost-button call-waiter-button"
        ...
        <button
          v-if="isLeader"
          class="primary-button confirm-order-button"
          :disabled="loading || !canConfirmComanda"
          @click="confirmComanda"
        >
          Confirmar
        </button>
      </footer>
