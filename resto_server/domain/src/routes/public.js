function parseMesaNumero(params) {
  const mesaNumero = Number.parseInt(params.mesaNumero, 10);

  if (!Number.isInteger(mesaNumero) || mesaNumero <= 0) {
    throw new Error('El numero de mesa es invalido');
  }
...
export async function publicRoutes(app, mesaService) {
  ...
  app.post('/internal/public/mesas/:mesaNumero/session', async (request, reply) => {
    ...
    const context = await mesaService.getMesaContext(mesaNumero, requestedClientSessionId);
  ...
  app.post('/internal/public/mesas/:mesaNumero/disconnect', async (request) => {
    ...
    return mesaService.disconnectClient(mesaNumero, clientSessionId);
  });
