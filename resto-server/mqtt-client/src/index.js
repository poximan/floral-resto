...
function getTunnelServiceUrl(pathname) {
  return new URL(pathname, process.env.TUNNEL_CLIENT_BASE_URL);
}
...
async function readTunnelState({ requestId = null, deviceId = null } = {}) {
  ...
  while (Date.now() <= deadline) {
    ...
    try {
      app.log.info(
        ...
        'mqtt-client consulta URL efimera a tunnel-client',
      );
      const response = await fetch(getTunnelServiceUrl('/entrypoint'));
      const payload = await response.json();

      if (!response.ok) {
        throw new Error(payload?.error ?? 'El tunnel-client rechazo la consulta de URL efimera');
      }
      ...
      app.log.info(
        ...
        'mqtt-client recibio estado de tunnel-client',
      );
      ...
      app.log.warn(
        ...
        'mqtt-client no pudo consultar tunnel-client',
      );
    }
  ...
  throw lastError ?? new Error('tunnel-client no entrego URL efimera vigente');
}
  ...
  try {
    ...
    await publishJson(topics.systemWebEntrypointCurrent, {
      ...
      error: tunnelState.ready && tunnelState.publicUrl
        ? null
        : tunnelState.lastError ?? 'tunnel-client aun no conoce una URL efimera vigente',
      source: 'tunnel-client:/entrypoint',
    });
    app.log.info(
      {
        requestId,
