...
const healthTargets = [
  ...
  { name: 'edge-gateway', url: 'http://servicoop-edge-gateway:8080/gateway-health' },
  { name: 'channel-adapter', url: 'http://channel-adapter:8080/health' },
  { name: 'tunnel-client', url: 'http://tunnel-client:8083/health' },
...
async function getTunnelEntrypoint() {
  try {
    const response = await fetch('http://tunnel-client:8083/entrypoint', {
      signal: AbortSignal.timeout(1500),
    });
    ...
    <script>
      ...
      function render(snapshot) {
        ...
        if (!snapshot.sessions.length) {
          document.getElementById('sessions').innerHTML += '<p class="hint">Solo se observan accesos que entran por edge-gateway mediante floral.internal. Si usas directamente localhost:5175 o localhost:5174, esa actividad no pasa por este dashboard.</p>';
        }
