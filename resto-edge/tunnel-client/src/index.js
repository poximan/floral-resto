...
function updateFromOutput(chunk) {
  ...
  app.log.info({ line }, 'Salida de tunnel-client');
...
app.get('/entrypoint', async () => {
  ...
  app.log.info(
    ...
    'tunnel-client entrega estado de entrypoint',
  );
...
await app.listen({
  host: process.env.TUNNEL_CLIENT_HOST,
  port: Number.parseInt(process.env.TUNNEL_CLIENT_PORT, 10),
});
